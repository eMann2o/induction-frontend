...
export default function SessionDetails() {
  ...
  // Fetch attempts when session first loads
  useEffect(() => {
    if (!session) return;
    fetchAttempts();
  }, [session?.id]);
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                      ...
                      {session.status === "active" ? "Live Results" : "Session Results"}
                    </h2>
                    {lastFetchTime && (
                  ...
                  >
                    <span className={`material-symbols-rounded ${loadingResults ? 'animate-spin' : ''}`}>
                      refresh
                    </span>
                    <span>{loadingResults ? 'Refreshing...' : 'Refresh'}</span>
                  </button>
                </div>

                {loadingResults && attempts.length === 0 ? (
                  <div className="text-center py-8">
                    <span className="material-symbols-rounded text-4xl text-gray-300 animate-spin">
                      progress_activity
                    </span>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 mt-2">
                      {session.status === "active" 
                        ? "Waiting for trainees to submit attempts..."
